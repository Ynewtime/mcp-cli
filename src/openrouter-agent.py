...
class TerminalSpinner:
    ...
    def spin(self):
        ...
        while self.running:
            if self.is_tty:
                sys.stdout.write(f"\r{self.spinner_chars[self.spinner_idx]} {self.message}")
                sys.stdout.flush()
...
class MCPClient:
    ...
    async def connect_to_server(self, server_config: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"成功连接到服务器，可用工具: {[tool.name for tool in response.tools]}")
    ...
    def _validate_tool_parameters(self, tool_name: str, tool_args: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """验证和修复工具参数
        ...
        # 特定工具的参数修复
        if tool_name == "list_directory" and "path" not in fixed_args and "input" in fixed_args:
            fixed_args["path"] = fixed_args.pop("input")
        elif tool_name == "search_files" and "pattern" not in fixed_args and "input" in fixed_args:
            fixed_args["pattern"] = fixed_args.pop("input")
        elif tool_name == "move_file" and ("source" not in fixed_args or "destination" not in fixed_args):
    ...
    async def process_query(self, query: str) -> str:
        ...
        while iterations < max_iterations:
            ...
            try:
                logger.debug(f"API请求原始数据: available_tools:{available_tools}; self.messages:{self.messages}")
            ...
            # 检查是否为直接响应
            if "Action:" not in content_text and "Thought:" not in content_text and not content.tool_calls:
                self._print_step(f"响应: {content_text}", level="success")
                break

            # 提取思考过程
            reasoning = re.search(r"Thought: (.*?)(?=Action:|$)", content_text, re.DOTALL)
            ...
            if content.tool_calls:
                ...
                for tool_call in content.tool_calls:
                    ...
                    try:
                        # 解析工具参数
                        tool_args = self._sanitize_json_input(tool_args_str) if tool_args_str and tool_args_str.startswith("{") else {"input": tool_args_str}
                        if tool_args is None:
                            raise ValueError("无效的JSON参数")
                            
                        self._print_step(f"执行操作: {tool_name}", level="action", with_spinner=True)
                        self._print_step(f"操作输入: {json.dumps(tool_args, indent=2, ensure_ascii=False)}", level="info")
                        ...
                        observation = result.content if hasattr(result, 'content') else str(result)
                        observation = self._sanitize_text(observation)
                        
                        self._print_step(f"观察结果: {observation}", level="observation")
                ...
                action_input = re.search(r"Action Input:\s*(.*?)(?=\n|Action:|$)", content_text, re.DOTALL | re.IGNORECASE)
        ...
        if iterations >= max_iterations:
            ...
            try:
                response = self.openai.chat.completions.create(
                    model=MODEL,
                    messages=self.messages + [{"role": "user", "content": "请提供最终答案。"}]
                )
                logger.debug(f"API响应原始数据: {response}")
                total_input_tokens += response.usage.prompt_tokens or 0
                total_output_tokens += response.usage.completion_tokens or 0
                ...
                final_answer = self._sanitize_text(
                    response.choices[0].message.content if response.choices else "无可用答案"
                )
        ...
        while True:
            try:
                ...
                if query.lower() in ["quit", "exit", "bye"]:
                    duration = (datetime.datetime.now() - session_start_time).total_seconds()
                    summary = f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 会话统计:
